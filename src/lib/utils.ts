...
const rankIcons = [
	{ id: 'NOXCREW', icon: 'noxcrew', name: 'Noxcrew' },
	{ id: 'MODERATOR', icon: 'moderator', name: 'Moderator' },
	{ id: 'CONTESTANT', icon: 'contestant', name: 'Contestant' },
	{ id: 'CREATOR', icon: 'creator', name: 'Creator' },
	{ id: 'GRAND_CHAMP_SUPREME', icon: 'grand_champ_supreme', name: 'Grand Champ Supreme' },
	{ id: 'GRAND_CHAMP_ROYALE', icon: 'grand_champ_royale', name: 'Grand Champ Royale' },
	{ id: 'GRAND_CHAMP', icon: 'grand_champ', name: 'Grand Champ' },
	{ id: 'CHAMP', icon: 'champ', name: 'Champ' }
];
...
export function getRankString(ranks: string[]) {
	for (const rank of rankIcons) {
		if (ranks.includes(rank.id)) {
			return rank.name;
		}
...
export function calculateTrophiesToNextEvolution(
	levelType: string,
	...
	for (let level = 0; level < nextEvolution; level++) {
		runningTotalForNextEvolution +=
			(levelType === 'crown' ? levels : levelType === 'style' ? styleLevels : fishingLevels).find(
...
export function earnedTrophiesForIsland(
	allFish: { fish: IslandFish; weights: IslandFishWeight[] }[],
	island: string
) {
	let runningTotal = 0;
...
export function totalTrophiesForIsland(
	allFish: { fish: IslandFish; weights: IslandFishWeight[] }[],
	island: string
) {
	let runningTotal = 0;
...
export function caughtWeightsForIsland(
	allFish: { fish: IslandFish; weights: IslandFishWeight[] }[],
	island: string
) {
	let runningTotal = 0;
		...
		if (trophies > runningTotal + tier.trophies * 10) {
			...
			for (let level = tier.range[0]; level <= tier.range[1]; level++) {
				if (trophies > runningTotal + tier.trophies) {
					...
					return {
						level,
						progressThroughLevel: trophies - runningTotal,
						totalForLevel: tier.trophies
					};
				}
			}
		}

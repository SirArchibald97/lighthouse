	...
	>
		<div class="flex flex-row gap-x-2 p-2">
			<div class="flex min-w-full justify-between gap-x-4">
				...
				<div
					class={`flex shrink-0 gap-x-1 self-start rounded-full px-2 py-0.5 ${getBadgeStat(badge.stat) > 0 ? 'bg-green-800' : 'bg-neutral-700/50'}`}
				>
					<img
		...
		<div
			class={`mt-auto flex flex-row rounded-b-md p-2 px-3 text-sm lg:text-base ${getBadgeStat(badge.stat) > 0 ? 'bg-green-800/50' : 'bg-neutral-800'}`}
		>
			{#if getBadgeStat(badge.stat) > 0}
				<p class="flex gap-x-1 text-green-500">
					<img
						src="https://cdn.discordapp.com/emojis/1042056406997663844.webp?size=96"
						alt="Check Icon"
						class="size-6 self-center"
					/>
					<span class="self-center">
						...
						<span class="font-semibold tabular-nums"
							>{getBadgeStat(badge.stat).toLocaleString()}</span
						>
			...
			{:else}
				<p class="flex gap-x-1 text-neutral-400">
					<img
						src="https://cdn.discordapp.com/emojis/1042056408423739492.webp?size=96"
						alt="Check Icon"
						class="size-6 self-center"
					/>

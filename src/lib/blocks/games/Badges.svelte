...
{#each badges as badge}
    <div class={`flex flex-col gap-y-2 text-lg border ${getBadgeStat(badge.stat) > 0 ? "bg-green-800/10 border-green-800/50" : "border-neutral-800"}`}>
        <div class="flex flex-row gap-x-2 p-2">
            <div class="flex gap-x-4 min-w-full justify-between">
                ...
                <div class={`flex shrink-0 gap-x-1 rounded-full px-2 py-0.5 self-start ${getBadgeStat(badge.stat) > 0 ? "bg-green-800" : "bg-neutral-700/50"}`}>
                    ...
                    <p class="text-base flex self-center">{badge.trophies}</p>
                </div>
            </div>
        </div>
        <div class={`flex flex-row justify-between rounded-b-lg mt-auto p-2 px-3 text-base ${getBadgeStat(badge.stat) > 0 ? "bg-green-800/50" : "bg-neutral-800"}`}>
            {#if getBadgeStat(badge.stat) > 0}
                <p class="flex gap-x-1 text-green-500">
                    <span class="size-6 "><Check /></span>
                    <span>Completed </span>

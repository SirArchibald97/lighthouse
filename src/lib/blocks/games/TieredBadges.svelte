	...
	>
		<div class="flex justify-between p-2">
			<div class="flex min-w-full justify-between gap-x-4">
				<div class="flex gap-x-2">
					...
					<div class="flex flex-col">
						<p class="font-semibold">
							{badge.name}
							{calculateBadgeTier(stats[badge.stat], badge.tiers).tier.name}
						</p>
				...
				>
					...
					/>
					<p class="flex self-center text-sm lg:text-base">
						{calculateTotalTrophies(stats, [], [badge])}
					</p>
				</div>
			</div>
		</div>
		...
		>
			...
			{#if badge.tiers[badge.tiers.length - 1].amount < stats[badge.stat]}
				<span class="self-center text-green-600">({stats[badge.stat].toLocaleString()})</span>
			{/if}
		</p>
	</div>
{/each}

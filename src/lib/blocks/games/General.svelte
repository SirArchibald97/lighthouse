<script lang="ts">
	...
	const gameTrophies = [
		...
		},
		{
			...
			trophies: calculateTotalTrophies(
				player.statistics?.tgttos!,
				badges.tgttos!,
				badges.tgttos_tiered
			),
...
<div
	class="flex flex-col divide-y divide-neutral-800 border-t border-neutral-800 px-4 text-base md:text-lg"
>
	<div class="flex flex-col gap-x-20 gap-y-4 py-4 lg:flex-row">
		<div>
			<p>
				Total XP: <span class="font-semibold tabular-nums"
					>{player.statistics?.total_xp.toLocaleString()}</span
				>
			</p>
			<p>
				Game XP: <span class="font-semibold tabular-nums"
					>{player.statistics?.game_xp.toLocaleString()}</span
				>
			</p>
			<p>
				Fishing XP: <span class="font-semibold tabular-nums"
					>{player.statistics?.fishing_xp.toLocaleString()}</span
				>
			</p>
			<p>
				Other XP: <span class="font-semibold tabular-nums"
					>{(
						player.statistics?.total_xp! -
						player.statistics?.game_xp! -
						player.statistics?.fishing_xp!
					).toLocaleString()}</span
				>
			</p>
		</div>
		<div>
			<p>
				Games Played: <span class="font-semibold tabular-nums"
					>{player.statistics?.games_played.toLocaleString()}</span
				>
			</p>
			...
			<p>
				...
				<span class="font-semibold tabular-nums"
					>{roundNumber(
						gamesWon / (player.statistics?.games_played! - gamesWon)
					).toLocaleString()}</span
				>
				<span class="text-neutral-500"
					>({calculatePercentage(gamesWon, player.statistics?.games_played!)}%)</span
				>
			</p>
			<p>
				Community Contributions: <span class="font-semibold tabular-nums"
					>{player.statistics?.community_contribution.toLocaleString()}</span
				>
			</p>
		</div>
	</div>
	...
	<div class="py-4">
		...
		<div class="flex flex-col gap-4 md:flex-row">
			<!-- skill trophies -->
			<div
				class="flex w-full flex-col items-center gap-4 rounded-md bg-neutral-900 p-2 md:w-1/2 xl:flex-row"
			>
				<span
					class="flex size-32 items-center justify-center rounded-full bg-neutral-100 md:size-44"
				...
				<div class="flex gap-x-4 rounded-md bg-neutral-900 p-2">
					<span
						...
						style="background: conic-gradient({getCrownColourHex(
							...
							(calculateTotalTrophies(game.stats!, game.badges, game.tieredBadges) /
								calculateMaxTrophies(game.badges, game.tieredBadges)) *
								360

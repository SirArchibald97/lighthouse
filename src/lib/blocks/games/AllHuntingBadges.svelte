<script lang="ts">
	...
	function calculateStatLeft(badge: IslandTieredBadge) {
		return forNextTier
			? calculateBadgeTier(stats[badge.game][badge.stat], badge.tiers).next.amount -
					stats[badge.game][badge.stat]
						...
						<div class="flex gap-x-2">
							...
							<div class="flex flex-col gap-y-1">
								<p class="font-semibold">
									{badge.name}: {calculateBadgeTier(stats[badge.game][badge.stat], badge.tiers).tier
										.name} -> {forNextTier
								...
								<p class="mt-2 text-sm text-neutral-300 lg:text-base">
									<span
										>Estimated {badge.game === 'parkour_warrior/solo' ? 'Completions' : 'Games'} Left:</span
									>
									<span class="font-semibold">
										...
										{:else}
											{(
												Math.ceil(
													calculateStatLeft(badge) /
														(stats[badge.game][badge.stat] / stats[badge.game].games_played)
												) || 'Unknown'
											).toLocaleString()}
										{/if}
									</span>
								...
								<p class="flex gap-x-1 text-sm text-neutral-300 lg:text-base">
									...
									<span class="font-semibold tabular-nums"
										>{calculateBadgeTier(stats[badge.game][badge.stat], badge.tiers).next
											.trophies}</span
									>

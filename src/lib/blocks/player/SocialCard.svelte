<script lang="ts">
	...
	function selectPage(index: number) {
		if (index < 0)
			index = Math.ceil(player?.social?.friends.length! / getSizeFromWidth(screen.width)) - 1;
		if (index >= Math.ceil(player?.social?.friends.length! / getSizeFromWidth(screen.width)))
			index = 0;
		currentIndex = index;
	}
</script>
...
<div class="flex h-full flex-col rounded-lg border border-neutral-800 shadow-lg">
	{#if player?.social}
			...
			{#each [{ label: 'Friends', icon: 'friend' }, { label: 'Party', icon: 'party' }] as { label: string; icon: string }[] as tab}
				...
				>
					<span class="flex flex-row gap-x-2">
						<img
							src="https://cdn.islandstats.xyz/icons/social/{tab.icon}.png"
							alt="{tab.label} Icon"
							class="size-6"
						/>
						<span>{tab.label}</span>
		...
		{#if activeTab === 'Friends'}
			<div
				class="grid grow grid-cols-1 grid-rows-5 gap-2 border-b border-neutral-800 p-3 md:grid-cols-2 2xl:grid-cols-3"
			>
				{#each player!.social!.friends.slice(currentIndex * getSizeFromWidth(screen.width), currentIndex * getSizeFromWidth(screen.width) + getSizeFromWidth(screen.width)) as friend}
					...
					>
						<div class="flex flex-row gap-x-2 self-center">
							<span class="relative">
								<img
									src="https://mc-heads.net/avatar/{friend.uuid}.png"
									alt="{friend.username}'s Skin"
									class="size-7 rounded-sm 2xl:size-8"
								/>
								{#if friend.status}
									<span
										class="absolute right-0 bottom-0 block translate-x-1/2 translate-y-1/2 transform rounded-full border-3 border-neutral-950"
									>
										<span
											class="block size-2 rounded-full {friend.status.online
												? 'bg-green-400'
												: 'bg-red-400'}"
										></span>
									</span>
								...
								{#if friend.status.server?.category === 'GAME'}
									<img
										src="https://cdn.islandstats.xyz/games/{getStatusIcon(
											friend.status.server?.associatedGame === 'PARKOUR_WARRIOR'
												? friend.status.server?.subType
												: friend.status.server?.associatedGame
										)}/icon.png"
			...
			<div class="flex h-12 justify-center gap-x-6">
				...
				<p class="self-center tabular-nums">
					{currentIndex + 1} / {Math.ceil(
						player!.social!.friends.length / getSizeFromWidth(screen.width)
					)}
				</p>
				<button
					onclick={() => selectPage(currentIndex + 1)}
		...
		{:else if activeTab === 'Party'}
					...
					{#each player.social.party.members as member}
						...
						>
							<div class="flex flex-row gap-x-2">
								<span class="relative">
									<img
										src="https://mc-heads.net/avatar/{member.uuid}.png"
										alt={``}
									...
									{#if member.status}
										...
										>
											<span
												class={`block size-2 rounded-full ${member.status.online ? 'bg-green-400' : 'bg-red-400'}`}
											></span>
										</span>
									{/if}
								</span>
								...
								{#if member.status.online}
									{#if member.status.server?.category === 'GAME'}
										<div class="flex flex-col">
											...
											>
												{getStatusString(
													member.status.server?.associatedGame === 'PARKOUR_WARRIOR'
														? member.status.server?.subType
														: member.status.server?.associatedGame
												)}
											</Tooltip>
										</div>
			...
			{:else}
				<div
					class="flex h-full flex-row items-center justify-center rounded-lg bg-neutral-950/40 py-8"
				>
					<div class="flex flex-row gap-x-2">
						<img
	...
	{:else}
		<div
			class="flex h-full flex-row items-center justify-center rounded-lg bg-neutral-950/40 text-neutral-400"
		>

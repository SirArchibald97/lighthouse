<script lang="ts">
	...
	import { daysSince, formatCaughtDate, getRankIcon, getStatusIcon, getStatusString } from '$lib/lighthouseUtils';
	...
	<div class="flex justify-between">
		<div class="flex gap-x-2 md:gap-x-4">
			<span class="relative inline-block">
				<img class="size-12 rounded-md md:size-16" src="https://crafatar.com/avatars/{player.uuid}?overlay" alt={`${player.username}'s Skin`} />
				{#if player.status}
					<span
						...
						<span class={`block size-2 rounded-full md:size-3 ${player.status.online ? 'bg-green-400' : 'bg-red-400'}`}></span>
					</span>
				{/if}
			</span>
				...
				<div class="flex w-full justify-between">
					<div class="flex gap-x-2">
						...
						{#if player.mccPlusStatus}
							<img
								src="https://cdn.islandstats.xyz/ranks/plus_{player.mccPlusStatus.evolution + 1}_simple.png"
								alt="MCC Plus Icon"
						...
						{#if player.status.server?.category === 'GAME'}
							<p class="flex flex-row gap-x-2">
								...
								<span class="font-semibold"
									>{getStatusString(
										player.status.server?.associatedGame === 'PARKOUR_WARRIOR' ? player.status.server?.subType : player.status.server?.associatedGame
									)}</span>
							</p>
		...
		{#if player.status}
			...
			<p class="size-7 cursor-pointer self-start rounded-md border border-neutral-800 p-0.5 duration-100 hover:bg-neutral-800"><Info /></p>
			<Tooltip
				arrow={false}
				type="custom"
				placement="top"
				...
				<p>
					First joined: {firstJoin.getHours() < 10 ? `0${firstJoin.getHours()}` : firstJoin.getHours()}:{firstJoin.getMinutes() < 10

	...
	<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
		<div class="flex h-16 justify-between">
			<div class="flex flex-row justify-between">
				...
				<div class="hidden *:duration-75 sm:-my-px sm:ml-6 sm:flex sm:space-x-8">
					{#each headers as header}
						<a
							href={header.href}
							class={`inline-flex items-center border-b-4 px-1 pt-1 text-sm font-medium ${page.url.pathname.includes(header.href) ? 'border-neutral-50 text-neutral-50' : 'border-transparent text-neutral-400 hover:border-neutral-100 hover:text-neutral-100'}`}
							aria-current="page">
							{header.label}
						</a>
					{/each}
			...
			<div class="flex flex-col justify-center gap-y-1 pt-2 pb-3">
				{#each headers as header}
					<a
						...
						href={header.href}
						class={`border-b-4 px-4 pt-1 text-sm font-medium ${page.url.pathname === header.href ? 'border-neutral-50 text-neutral-50' : 'border-transparent text-neutral-400 hover:border-neutral-100 hover:text-neutral-100'}`}
						aria-current="page">
						{header.label}
					</a>
				{/each}

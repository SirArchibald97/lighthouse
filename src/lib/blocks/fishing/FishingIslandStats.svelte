				...
				<p class="text-base">
					<span
						>{player.collections!.fish.filter(
							(f) => f.fish.collection === collection.name && f.weights.length > 0
						).length}</span
					>
					<span> / </span>
					<span

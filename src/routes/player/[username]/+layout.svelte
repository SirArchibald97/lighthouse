		...
		{:then player}
			...
			{:else}
				<div class="animate-fade flex w-full flex-col">
					...
					<div class="mt-4 flex flex-col divide-y divide-neutral-800 rounded-t-md border border-neutral-800 lg:hidden">
						<button type="button" onclick={() => (selectMenuOpen = !selectMenuOpen)} class="flex w-full cursor-pointer justify-between p-4 text-neutral-200">
							<div>
								<p class="flex gap-x-2">
									<img
										src={currentTab?.icon.startsWith('/') ? currentTab.icon : `https://cdn.islandstats.xyz/${currentTab?.icon}.png`}
							...
							<div transition:slide={{ duration: 100 }} class="flex flex-col justify-center gap-y-3 p-4">
								{#each tabs as tab}
									<a
										...
										aria-current="page">
										<p class="flex gap-x-2">
											<img src={`https://cdn.islandstats.xyz/${tab.icon}.png`} alt={`${tab.alt} Icon`} class="size-6" />
